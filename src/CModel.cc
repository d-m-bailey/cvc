...
returnCode_t CModelListMap::SetVoltageTolerances(teestream & theReportFile, CPowerPtrMap & thePowerMacroPtrMap) {
	...
	for (CModelListMap::iterator modelList_pit = begin(); modelList_pit != end(); modelList_pit++) {
		for (CModelList::iterator model_pit = modelList_pit->second.begin(); model_pit != modelList_pit->second.end(); model_pit++) {
			try {
				if ( ! IsEmpty(model_pit->maxVbgDefinition) ) model_pit->maxVbg = thePowerMacroPtrMap.CalculateVoltage(model_pit->maxVbgDefinition, SIM_POWER, (*this));
				if ( ! IsEmpty(model_pit->maxVbsDefinition) ) model_pit->maxVbs = thePowerMacroPtrMap.CalculateVoltage(model_pit->maxVbsDefinition, SIM_POWER, (*this));
				if ( ! IsEmpty(model_pit->maxVdsDefinition) ) model_pit->maxVds = thePowerMacroPtrMap.CalculateVoltage(model_pit->maxVdsDefinition, SIM_POWER, (*this));
				if ( ! IsEmpty(model_pit->maxVgsDefinition) ) model_pit->maxVgs = thePowerMacroPtrMap.CalculateVoltage(model_pit->maxVgsDefinition, SIM_POWER, (*this));
			}
